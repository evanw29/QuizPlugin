...
public function process_recommendations($quiz_id) {
    ...
    // Step 1: Fetch user responses for this quiz
    ...
    // Step 2: Map answers to keywords and their weights
    ...
    if (empty($triggered_keywords)) {
        error_log("No keywords triggered for QuizID: $quiz_id");
        return [];
    }

    // Step 3: Map keywords to technologies with weighted scoring
    $tech_scores = [];
    ...
    // Step 4: Sort technologies by score (descending)
    arsort($tech_scores);

    // Step 5: Apply diversity constraint
    $top_tech_ids = [];
